...
const ProductsPage = () => {
  ...
  return (
    <div className='dark:bg-gray-900 dark:text-white z-5'>
      <ProductOperations />
      <div className="grid lg:grid-cols-4 lg:grid-rows-3 md:grid-cols-3  sm:grid-cols-2  gap-2 bg-white text-xl my-4  dark:bg-gray-800 md:mx-2 lg:mx-3 xl:mx-4 dark:text-gray-200">
        {loading ? (
          <p className="col-span-3">loading...</p>
        ) : (
          products &&
          products.products.map((product) => (
            <div key={product.id} className='rounded-lg p-2 border border-blue-200 dark:border-blue-500 ms:p-2 relative dark:bg-gray-700 group'>

              <img src={'./../../../galler.icon.jpeg'} alt="" className='rounded-lg' />
              <div className=' p-2'>
              ...
              <span className='absolute flex items-center p-1 text-white  bg-gray-100 text-blue-400 rounded-lg right-3 top-5 opacity-0 cursor-pointer group-hover:opacity-100 transition-opacity'>
                <button>
